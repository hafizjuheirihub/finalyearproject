...
<section id="customerTable">
	...
	<thead>
		...
		<tr>
			...
			<td><c:out value="${r.petType}"/></td>
			<td>
    <c:choose>
        <c:when test="${r.status == 'Pending'}">
            <span class="badge badge-warning" style="font-size: 14px; font-weight: normal; padding: 12px;">
                <c:out value="${r.status}"/>
            </span>
        </c:when>
        <c:when test="${r.status == 'Approve'}">
            <span class="badge badge-success" style="font-size: 14px; font-weight: normal; padding: 12px;">
                <c:out value="${r.status}"/>
            </span>
        </c:when>
        <c:when test="${r.status == 'Reject'}">
            <span class="badge badge-danger" style="font-size: 14px; font-weight: normal; padding: 12px;">
                <c:out value="${r.status}"/>
            </span>
        </c:when>
        <c:otherwise>
            <span class="badge badge-secondary" style="font-size: 14px; font-weight: normal; padding: 12px;">
                <c:out value="${r.status}"/>
            </span>
        </c:otherwise>
    </c:choose>
</td>
			<td class="wrap-text"><c:out value="${r.remarks}"/></td> 
			<td><a class="btn btn-primary" href="listAnswersController?RequestID=<c:out value="${r.requestID}"/>"> <i class="fa fa-eye"></i></a></td>
			<%-- <td><a class="btn btn-primary" href="viewStaffController?StaffID=<c:out value="${s.staffID}"/>">View</a></td>
			<td><a class="btn btn-primary" href="updateStaffController?StaffID=<c:out value="${s.staffID}"/>">Update</a></td>
